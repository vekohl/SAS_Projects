...
(
 Select g.ncDOBYear as [Year], g.nStateFileNumber as [SFN],
       ...
       --And s.cDepartment<>'VR State' and v.cuid <> 'BOBO'
	   AND v.cuid NOT IN (SELECT cuserid FROM securityuserlocation WHERE cLocationID = '1')
...
data ck; 
 set audit1;
  ...
  if substr(department,1,3) EQ 'CSE';
  *time=timepart(timeviewed); *format time time.;
...
%excelout (set=final_keep, file=SteveB\QuarterlyAuditFiles\CS_audit_file_&qtr..xlsx, sheet=&qtr);
